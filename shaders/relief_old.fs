...
void main()
{
    ...
    vec3 P = vec3(r_tex_coord, depth);
    vec3 At = position_tan;
    vec3 Pt = At + -n_to_eye * (1.0/n_to_eye.z) * scale * depth;
    vec3 p_to_light = light_position_tan - Pt;
    vec3 light_in = P + (p_to_light * 1.0/p_to_light.z) * scale * depth;

    float light_depth = findDepth(vec2(light_in), vec2(P));
    if(light_depth < depth-0.05) //in shadow
    //if(light_depth < 0.95) //in shadow
        gl_FragColor = vec4(0.0, 0.0, 0.0, 1.0);
    else
        gl_FragColor = col;
    //gl_FragColor = vec4(light_in, 1.0);
